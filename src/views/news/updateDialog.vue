  ...
  <el-dialog title="修改新闻" :visible.sync="dialogFormVisible" width="1200px">
    <el-form :model="formNews" class="news_form">
      ...
      <el-form-item label="发布时间" class="news_time">
        <el-date-picker v-model="formNews.addtime" type="date" placeholder="选择日期">
        </el-date-picker>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    // 编辑新闻接口
    async newsUpdate() {
      let res = await this.$API.API.updateNewsInfo(this.formNews)
      if (res.code == 1) {
        this.$message({
          message: '修改成功',
          type: 'success',
  ...
  beforeCreate() {
    this.$bus.$off('updateDialog')
    this.$bus.$off('updateId')
  },
  mounted() {
    let uPdateDialog = value => {
      this.dialogFormVisible = value
    }
    this.$bus.$on('updateDialog', uPdateDialog)
    this.$bus.$on('updateId', value => {
